...
export default function Inventory() {
  ...
  const initialFormState = { 
    id: null, name: '', category: 'Geral', price: '', cost: '', stock: '', minStock: 5, unit: 'UN', supplier: '', expiryDate: ''
  };
  
  const [formData, setFormData] = useState(initialFormState);
  const isAdmin = user?.role === 'admin'; // <--- Variável de controle
  ...
  return (
    <div className="p-6 pb-24 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                        ...
                        <input className="w-full border p-2 rounded dark:bg-slate-950" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required autoFocus />
                    </div>
                    <div>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar..." className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
              ...
              <div key={product.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-indigo-300 transition">
                <div className="flex-1">
                    ...
                    <div className="text-sm text-slate-500 dark:text-slate-400 flex gap-4 mt-1">
                        {/* Se quiser esconder o CUSTO do caixa também, coloque isAdmin aqui */}
                    ...
                    {/* SÓ ADMIN VÊ FORNECEDOR */}
                    {isAdmin && product.supplier && (
                        <p className="text-xs text-slate-400 mt-1 flex items-center gap-1"><Truck size={10}/> {product.supplier}</p>
                    )}
                </div>
